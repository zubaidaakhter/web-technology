...
<body style= "text-align:center;">
	<header><h1> Welcome to Manage Medicine </h1></header>
...
<?php
      ...
      if ($_SERVER['REQUEST_METHOD'] ==='POST') {
        ...
        if (empty($number) or empty($Medicine) or empty($description) or empty($price) || empty($stock)){
         ...
         $handle = fopen("../model/data2.json", "a");
          $arr1 = array('number'=>$number,'Medicine'=>$Medicine,'description'=>$description,'price'=>$price,'stock'=>$stock);
...
?>
<?php
    $handle = fopen("../model/data2.json", "r");
    $data = fread($handle, filesize("../model/data2.json"));
    $exploded = explode("\n", $data);
